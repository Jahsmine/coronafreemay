...
from flaskblog.models import UserModel


class RegistrationFormModel(FlaskForm):
...
class UpdateAccountFormModel(FlaskForm):
    username = StringField('Username',
                           validators=[DataRequired(), Length(min=4, max=20)]
                           )
    email = StringField('Email',
...
class RequestResetFormModel(FlaskForm):
    email = StringField('Email',
                        validators=[DataRequired(), Email()]
                        )
...
class ResetPasswordFormModel(FlaskForm):
